...
--
-- Set 0.8726 to the USD -> EUR rate of the day before running, e.g.
...
-- Listing prices ------------------------------------------------------------
UPDATE property
   SET base_price_usd   = round(base_price_usd   * 0.8726, 2),
       cleaning_fee_usd = round(cleaning_fee_usd * 0.8726, 2);

UPDATE listing
   SET nightly_usd = round(nightly_usd * 0.8726, 2);

-- Per-night calendar overrides ----------------------------------------------
UPDATE calendar_day
   SET price_usd = round(price_usd * 0.8726, 2)
 WHERE price_usd IS NOT NULL;

-- Bookings that have not been paid yet --------------------------------------
UPDATE booking
   SET currency      = 'EUR',
       nightly_usd   = round(nightly_usd   * 0.8726, 2),
       base_subtotal = round(base_subtotal * 0.8726, 2),
       subtotal      = round(subtotal      * 0.8726, 2),
       cleaning_fee  = round(cleaning_fee  * 0.8726, 2),
       service_fee   = round(service_fee   * 0.8726, 2),
       taxes         = round(taxes         * 0.8726, 2),
       total_usd     = round(total_usd     * 0.8726, 2)
